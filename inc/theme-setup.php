 ...
 */
function erlebnisbad_theme_setup() {
...
add_action( 'after_setup_theme', 'erlebnisbad_theme_setup' );

/**
 * Registers widgetized areas.
 */
function erlebnisbad_register_sidebars() {

	register_sidebar(
		array(
			'name'          => __( 'Footer', 'erlebnisbad' ),
			'id'            => 'footer',
	...
	register_sidebar(
		array(
			'name'          => __( 'Header Language Switcher', 'erlebnisbad' ),
			'id'            => 'header_ls',
...
add_action( 'widgets_init', 'erlebnisbad_register_sidebars' );
...
if ( ! function_exists( 'erlebnisbad_acf_google_maps_key' ) ) :
	/**
	 * Sets the Google Maps API key for ACF map fields.
	 * Requires erlebnisbad_GOOGLE_MAPS_API_KEY to be set in functions.php.
	 */
	function erlebnisbad_acf_google_maps_key() {
		if ( defined( 'erlebnisbad_GOOGLE_MAPS_API_KEY' ) && erlebnisbad_GOOGLE_MAPS_API_KEY ) {
			acf_update_setting( 'google_api_key', erlebnisbad_GOOGLE_MAPS_API_KEY );
		}
	}
endif;

add_action( 'acf/init', 'erlebnisbad_acf_google_maps_key' );
 ...
 * @return string
 */
function erlebnisbad_lower_yoast_metabox_priority() {
	return 'core';
}
